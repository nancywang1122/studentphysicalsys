...
keel.UploadPanel = function(cfg){
	...
	this.gp = new Ext.grid.GridPanel({
	    ...
	    columns: [
	        ...
	        {header: '文件名', width: 120, sortable: true,dataIndex: 'name', menuDisabled:true},
	        {header: '类型', width: 50, sortable: true,dataIndex: 'type', menuDisabled:true},
	        {header: '大小', width: 65, sortable: true,dataIndex: 'size', menuDisabled:true,renderer:this.formatFileSize},
	        {header: '进度', width: 160, sortable: true,dataIndex: 'percent', menuDisabled:true,renderer:this.formatProgressBar,scope:this},
	        {header: '状态', width: 65, sortable: true,dataIndex: 'state', menuDisabled:true,renderer:this.formatFileState,scope:this},
	...
	keel.UploadPanel.superclass.constructor.call(this,{				
		tbar : [
			...
			{text:'批量上传',ref:'../uploadBtn',iconCls:'up',handler:this.startUpload,scope:this},'-',
			//{text:'停止上传',ref:'../stopBtn',iconCls:'stop',handler:this.stopUpload,scope:this,disabled:true},'-',
			{text:'清空列表',ref:'../deleteBtn',iconCls:'trash',handler:this.deleteAll,scope:this},'-',
			{text:'<font color=red>提示:上传过程中,请不要打断,如需执行其它操作,请打开新的IE!</font>',scope:this},'-'
		],
		layout : 'fit',
		items : [this.gp],
		listeners : {
...
Ext.extend(keel.UploadPanel,Ext.Panel,{
	...
	formatDelBtn : function(v){
		return "<a href='javascript:void()'  id='"+v+"'  style='color:blue' class='link-btn' ext:qtip='移除该文件'>清空</a>";
	},
